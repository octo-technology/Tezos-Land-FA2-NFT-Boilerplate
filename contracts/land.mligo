...
let buy(p, s : buy_param * nft_token_storage) : (operation  list) * nft_token_storage =
    // TODO : verify amount is equal land price in tz
    let buyAmount : tez = Tezos.amount in
    let priceLand : price = match Big_map.find_opt p.id s.market.to_sell with
    | None -> (failwith("Land is not on sale"): price)
    | Some pl -> pl
    in
    if not (priceLand = buyAmount) then
       (failwith("The amount sent is not equal to the price of the land") : (operation  list) * nft_token_storage)
        ...
        | Some ownr -> ownr
        in
        let receiver : unit contract = match (Tezos.get_contract_opt o: unit contract option) with
        | Some (contract) -> contract
        | None -> (failwith ("Not a contract") : unit contract)
        in
        let buyTransaction : operation = Tezos.transaction unit buyAmount receiver in
        ...
        [ buyTransaction ], { s with ledger = new_ledger; market = { s.market with to_sell=new_to_sell }}
